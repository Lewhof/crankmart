...
/**
 * GET /api/community/lost?status=approved&province=&brand=&page=1
 *
 * Public read of approved lost-bike reports. Country-scoped so each region
 * sees its own. Filters keep the UI's facet bar honest without taking on
 * a search dependency.
 */
export async function GET(req: NextRequest) {
  ...
  const provinceClause = province ? sql`AND last_seen_location ILIKE ${'%' + province + '%'}` : sql``
  ...
  const [rowsRes, countRes] = await Promise.all([
    db.execute(sql`
      SELECT
        ...
        description, reward_text, status, created_at
      FROM lost_reports
